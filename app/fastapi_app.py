...
import Fonctions as helpers
import schemas

api_description = """
Welcome to the Movie Management API

This API allows you to interact with a database inspired by the famous [MovieLens](https://grouplens.org/datasets/movielens/) dataset.  
It is ideal for learning how to use a REST API with data on movies, users, ratings, tags, and external links (IMDB, TMDB).
...
- Get IMDB/TMDB links for a movie
- View overall statistics on the database

All endpoints support pagination (`skip`, `limit`) and optional filters as appropriate.

### Good to know
- You can test all endpoints directly via the Swagger interface “/docs”.
- For any error (e.g., non-existent ID), a clear response is returned with the correct HTTP code.
"""

# --- Initialisation de l'application FastAPI ---
app = FastAPI(
    title="API for Movie Management",
...
async def root():
    return {"message": "API for Movie Management opérationnelle"}
